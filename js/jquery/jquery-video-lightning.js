(function (jQuery, window, document) {
    ...
    function JQVideoLightning(element, options) {
        ...
        this.base_settings = jQuery.extend({}, defaults, options);
        this.init();
    }
    ...
    JQVideoLightning.prototype = {
        init: function () {
            ...
            if (jQuery("style:contains('.video-wrapper')").length < 1) {
            ...
            function callPlayer() {
                jQuery(this.player(target, target_wrapper, video_id, vendor, popover));
            }
            ...
            function resizeTail() {
                ...
                target_wrapper.find(".video-popover-tail").attr({
                    points: jQuery.proxy(tailPosition, this)
                });
            }

            target_wrapper.on("click", jQuery.proxy(callPlayer, this));

            jQuery(document).on("scroll mousewheel DOMMouseScroll MozMousePixelScroll", jQuery.proxy(resizeTail, this));
        ...
        settings: function () {
            ...
            function remapSettings(settings) {
                jQuery.each(settings,
                    jQuery.proxy(function (key, value) {
                        ...
                        jQuery(this).data(setting_key, value);
                    }, this)
                    );
            ...
            if (settings.videoWidth > jQuery(window).width() - 30) {
                settings.videoWidth = jQuery(window).width() - 30;
        ...
        popoverPosition: function (settings) {
            ...
            target = jQuery(this.element);
            target_wrapper = target.parent(".video-target");
            target_position_x = target_wrapper.offset().left - jQuery(window).scrollLeft();
            target_position_y = target_wrapper.offset().top - jQuery(window).scrollTop();
            window_center_x = jQuery(window).width() / 2;
            window_center_y = jQuery(window).height() / 2;
            ...
            function positionValues(positions) {
                return jQuery.map(positions, function (value) {
                    return value;
                });
            }
            ...
            function optimalPosition(positions, video_center, window_center) {
                ...
                jQuery.each(tempValues, function (i, value) {
                    jQuery.each(positions, function (list_key, list_value) {
                        if (list_value  === value) {
        ...
        coverImage: function (vendor, video_id, target_wrapper) {
            ...
            if (vendor === "youtube") {
                jQuery("<img class='video-cover'>").attr("src", youtube_img_url).appendTo(target_wrapper);
            } else {
                jQuery.getJSON(vimeo_api_url, {format: "jsonp"})
                    .done(function (data) {
                        jQuery("<img class='video-cover'>").attr("src", data[0].thumbnail_large).appendTo(target_wrapper);
                    });
            }
        },

        youtubePlayer: function (settings, video_id) {
            return jQuery("<iframe src=\"https://www.youtube.com/embed/" + video_id +
        ...
        vimeoPlayer: function (settings, video_id) {
            return jQuery("<iframe src=\"http://player.vimeo.com/video/" + video_id +
                "?autoplay=" + settings.videoAutoplay +
                "&loop=" + settings.videoLoop +
    ...
    jQuery.fn[pluginName] = function (options) {
        return this.each(function () {
            if (!jQuery.data(this, "plugin_" + pluginName)) {
                jQuery.data(this, "plugin_" + pluginName, new JQVideoLightning(this, options));
            }
        });
    };
